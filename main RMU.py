...
def get_utility(code, graph, isomorphism, R):
    R_dict = dict()
    for i in R:
        R_dict[i] = 0
    code_dict = dict()
    for u, v, _, _, _ in code:
        code_dict[(u,v)] = 0
        code_dict[(v,u)] = 0

    neg = 0
    rev_isomorphism = rev_map(isomorphism)
    for u in rev_isomorphism:
        if rev_isomorphism[u] in R_dict:
            continue
        for e in graph.adjacency[u]:
            v, edge_label, int_utility = e
            def find_edge_in_code(code, u , v):
                if (u,v) in code_dict:
                    return True
                return False
            if v not in rev_isomorphism or (u > v and not find_edge_in_code(code, rev_isomorphism[u], rev_isomorphism[v])):
            ...
            for u in graph.adjacency:
                for e in graph.adjacency[u]:
                    ...
                    if (0, 1, u_label, v_label, edge_label) in temp_result:
                        temp_result[(0, 1, u_label, v_label, edge_label)] = max(utility, temp_result[(0, 1, u_label, v_label, edge_label)][0]), graph.graph_utility()
                    else:
                        temp_result[(0, 1, u_label, v_label, edge_label)] = utility, graph.graph_utility()
        else:
                ...
                for v in R:
                    ...
                    for e in graph.adjacency[iso_u]:
                        ...
                        if not exists:
                            ...
                            utility, neg = get_utility(new_code, graph, isomorphism, R)
                            if (u, v, graph.vertices[iso_u], graph.vertices[iso_u], edge_label) in temp_result:
                                temp = temp_result[(u, v, graph.vertices[iso_v], graph.vertices[iso_v], edge_label)]
                                temp_result[(u, v, graph.vertices[iso_u], graph.vertices[iso_v], edge_label)] = max(utility, temp[0]), max(graph.graph_utility()-neg, temp[1])
                            else:
                                temp_result[(u, v, graph.vertices[iso_u], graph.vertices[iso_v], edge_label)] = utility, graph.graph_utility() - neg
                #Forward
                ur = u
                for u in R:
                    iso_u = isomorphism[u]
                    for e in graph.adjacency[iso_u]:
                        ...
                        utility, neg = get_utility(new_code,graph,isomorphism_, RightMostPath(new_code)[1])

                        if (u, ur+1, u_label, v_label, edge_label) in temp_result:
                            temp = temp_result[(u, ur+1, u_label, v_label, edge_label)]
                            temp_result[(u, ur+1, u_label, v_label, edge_label)] = max(utility, temp[0]), max(graph.graph_utility() - neg, temp[1])
                        else:
                            temp_result[(u, ur+1, u_label, v_label, edge_label)] = utility, graph.graph_utility() - neg

        for key in temp_result:
            temp = temp_result[key]
            if key in result:
                utility, gwu, tg = result[key]
                result[key] = (temp[0] + utility, gwu + temp[1], tg + graph.utility)
            else:
                result[key] = (temp[0], temp[1], graph.utility)
    return result
...
def GSpan(code, graphs, min_util, t):
    ...
    for key in extentions:
        utility, gwu, tg = extentions[key]
        new_code = copy.deepcopy(code)
        new_code.append(key)
        print(time.time()-t)
        #print(new_code, utility, gwu)
        cannonical = isCannonical(new_code)
        if cannonical and gwu > min_util:
            if utility > min_util:
                #print(new_code, utility, gwu, isCannonical(new_code))
                hup += 1
            GSpan(new_code, graphs, min_util, t)
        elif cannonical and tg > min_util:
            win += 1
            print("----------------------------WIN_________________________________________-----------------------")
